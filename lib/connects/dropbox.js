...
function callAPI(session, path, data, subdomain = 'api', isJson = true, headers = null) {
	const authorization = 'Bearer ' + session.credentials.access_token;
...
class DropboxConnector {
	...
	login(loginInfos) {
		...
		return returnPromise.then((access_token) => {
	...
	stat(path) {
		...
		.then((stat) => {
			return {
				...
				mimeType: Mime.getType(stat.name)
			};
		});
	}
	...
	move(src, dest) {
		...
		if(!dest)
			return Promise.reject(new UnifileError(UnifileError.EINVAL, 'Cannot move path with an empty destination'));
