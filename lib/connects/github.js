...
class GitHubConnector {
	...
	login( loginInfos) {
		...
		if(loginInfos.constructor === String) {
			...
			return this.setCredentials(session, `Basic ${new Buffer(url.auth).toString('base64')}`);
			...
			return this.setCredentials(session, `Basic ${auth}`);
			...
			.then((token) => {
				return this.setCredentials({access_token : `token ${token}`});
			});
		} else {
	...
	setCredentials(credentials) {
		...
		if(!credentials.access_token.startsWith('token ') && !credentials.access_token.startsWith('Basic '))
			return Promise.reject(new UnifileError(
				UnifileError.EACCES,
	...
	mkdir( path) {
		...
		switch (splitPath.length) {
			...
			case 1: // Create a repo
				...
				// Move default README to a more discreet .gitkeep
				.then(() => this.move(session, path + '/master/README.md', path + '/master/.gitkeep'));
	...
	move( src, dest) {
		...
		if(!dest) return Promise.reject(new Error('Cannot move path with an empty destination'));
		const splitPathDest = getPathTokens(dest);
		let apiPath;
		switch (splitPath.length) {
			case 0: // Error
				return Promise.reject(new UnifileError(UnifileError.EINVAL, 'Cannot move path with an empty name.'));
			case 1: // Move repo
				apiPath = '/repos/' + session.account.login + '/' + splitPath[0];
				const reqData = {name: dest};
				return this[callAPI](session, apiPath, reqData, 'PATCH');
			case 2: // Move branch (actually copy src to dest then remove src)
	...
	batch( actions, message) {
		...
		.filter((action) => ['rmdir', 'unlink', 'mkdir', 'writefile', 'move'].indexOf(action.name.toLowerCase()) > -1);
						...
						case 'move':
							if(!action.destination)
								return Promise.reject(new BatchError(
									UnifileError.EINVAL,
									'Move actions should have a destination'));
							actionsChain = actionsChain.then(() => this.move(session, action.path, action.destination))
							.catch((err) => err.name !== 'BatchError',
								(err) => {
	...
	[callAPI](session,path, data, method, isStream = false, retry = true) {
		const reqOptions = {
			...
			headers: {
				...
				'Authorization': session.credentials.access_token,
				'User-Agent': 'skynode-ufs',
