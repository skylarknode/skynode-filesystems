...
module.exports.mergeInfos = function(newInfos, defaults) {
	...
	// Do not use assign since we don't want any additional property
	return {
...
module.exports.simpleBatch = function(connector, session, actions) {
	return Promise.each(actions, (action) => {
		...
		switch (act) {
			...
			case 'rmdir':
			case 'mkdir':
				return connector[act](session, action.path);
			case 'rename':
				return connector[act](session, action.path, action.destination);
			case 'writefile':
				return connector.writeFile(session, action.path, action.content);
			default:
 ...
 * @param {Object|string} auth - Object or string containing auth
...
module.exports.encodePath = function(id) {
