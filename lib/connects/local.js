    ...
    return new Promise(function (resolve, reject) {
      existsWrapper(false, realSrcPath, () => {
        existsWrapper(true, realDestPath, () => {
          _fs.access(m_m_path.dirname(realDestPath), nfs.W_OK, (err) => {
            if ( err ) {
  ...
  info: function(session,resourceId) { //TODO
    ...
    return nfs.stat(realPath).then(function(stat){
      ...
      if (r.mime.indexOf('image/') == 0) {
        ...
        var tmbPath = m_path.join(config.tmbroot, filename + ".png");
      ...
      if (r.isdir) {
        ...
        for (var i = 0; i < items.length; i++) {
          if (nfs.lstatSync(m_path.join(p, items[i])).isDirectory()) {
              r.dirs = 1;
              break;
            }
        }
