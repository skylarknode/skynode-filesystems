...
function statToFileInfos(id,path, stat) {
...
class LocalConnector {
  ...
  move(session,srcPath, destPath) { //TODO
    const realSrcPath = this.toRealPath(srcPath);
    const realDestPath = this.toRealPath(destPath);
  ...
  stat (session, id) { 
    ...
    try {
      ...
      .then((stat) => {
        return statToFileInfos(id,path, stat);
      });
    } catch (e) {
      return Promise.reject(e);
    }
  ...
  writeFile(session,path, data,options) { 
    const realPath = this.toRealPath(path);
    options = options || {};
