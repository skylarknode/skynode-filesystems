...
grest.toUnifiedFileInfo = function(file) {
...
class GoogleDriveConnector {
    ...
    load(session) {
        ...
        if (session.tokens) {
            this.setAccessToken(session.tokens);
        }
    }
	getInfos() {
	...
	login(loginInfos) {
		...
		return returnPromise.then((tokens) => {
			return this.setAccessToken(tokens);
		});
	}
...
*/
    ...
    async readdir(path,options) {
        ...
        const readFiles = async params => {
            ...
            for(const file of result.files) {
                files.push(grest.toUnifiedFileInfo(file));
            }
    ...
    async stat(path) {
        ...
        if(path === "/") {
            ...
            return grest.toUnifiedFileInfo(res.result);
        }
        const parentFolder = await this.getFileOfPath(absPath.getPathPart());
        ...
        return grest.toUnifiedFileInfo(file);
    }
