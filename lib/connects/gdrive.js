 ...
 * @type {string}
 */
gapi.mimeTypeFolder = "application/vnd.google-apps.folder";
 ...
 */
gapi.getActualRootFolderId = async (session) => {
    ///const res = await gapi.getFileResource(session,{ fileId: "root", fields: "id" });
 ...
 */
gapi.getFileList =  async function (session,queryParameters) {
    const auth = session.auth;
    const response = await session.drive.files.list(queryParameters);
 ...
 */
gapi.findFolderByName = async function (session,parentFolderId, folderName) {
    debug("No tests pass: gapi.findFolderByName");
    const folders = [];
    const q = [
        `parents in '${parentFolderId}'`,
        `name = '${folderName}'`,
        `mimeType = '${gapi.mimeTypeFolder}'`,
        "trashed = false",
    ].join(" and ");
    ...
    try {
        do {
            const result = await gapi.getFileList(session,params);
 ...
 */
gapi.findFileByName =  async function (session,parentFolderId, fileName,options){
    debug("No tests pass: gapi.findFileByName");
    options = options || {}
    const files = [];
    const conds = [
        `parents in '${parentFolderId}'`,
 ...
 * @returns {Promise<object>} The result of the API.
 */
gapi.getFileResource = async function (session,parameters)  {
    const response = await session.drive.files.get(parameters);
 ...
 */
gapi.isFolder =  function(file) {
    return file.mimeType === gapi.mimeTypeFolder;
};
 ...
 */
gapi.downloadFile = async function (session,fileId, acknowledgeAbuse) {
    return gapi.requestWithAuth(session,"GET",
 ...
 */
gapi.createFile = async function (session,folderId, filename, mimeType){
    const response = await gapi.requestWithAuth(session,"POST",
 ...
 * @returns {Promise<object>} The response of the API.
 */
gapi.updateFile = async (session,fileId, mimeType, data) => {
    const response = await gapi.requestWithAuth(session,"PATCH",
 ...
 */
gapi.requestWithAuth =  function (session,method, endpoint, queryParams, headers, body){
    ...
    xhr.setRequestHeader("Authorization",
        "Bearer " + session.auth.credentials.access_token);
    xhr.timeout = 30000;
...
gapi.toUnifiedFileInfo = function(file) {
    const info = Object.assign(file);

    info.isDir = gapi.isFolder(file);
    return info;
}
...
class GoogleDriveConnector {
	...
	setAccessToken(session, tokens) {
        ...
        this.session = {};
        this.session.auth = this[oauth2Client];
        this.session.drive = google.drive({version: 'v3', auth:this.session.auth});
	...
	login(session, loginInfos) {
		...
		if(typeof loginInfos === 'object' && 'code' in loginInfos) {
			returnPromise = new Promise((resolve, reject) => {
				try {
            		...
            		///resolve(tokens);
            	    this[oauth2Client].getToken(loginInfos.code).then(function(result){
		...
		return returnPromise.then((tokens) => {
			return this.setAccessToken(session, tokens);
		});
	}
...
*/
    async copy(session,src,dest) { // only file TODO: support for dir
        debug(`copy (${src},${dest})`);
        const srcAbsPath = this.toAbsolutePath(new GdfsPath(src));
        const srcParentFolder = await this.getFileOfPath(session,srcAbsPath.getPathPart());
        ...
        const srcFiles = await gapi.findFileByName(this.session,srcParentFolder.id, srcPathName);
        ...
        const destParentFolder = await this.getFileOfPath(session,destAbsPath.getPathPart());
        ...
        const destFiles = await gapi.findFileByName(this.session,destParentFolder.id, destPathName);
        ...
        // Create Copy of File
        const cloned = (await this.session.drive.files.copy({
            fileId: srcFile.id
        })).data;
        ...
        const response = await this.session.drive.files.update(params);
     ...
     */
    async isDirectory (session,path) {
      debug("No tests pass: Gdfs#isDirectory");
      const file = await this.getFileOfPath(session,path);
    ...
    async getPaths (session,path) {
      ...
      for (const name of path.elements().slice(1)) {
        ...
        if (parent.id != null) {
          const children = await gapi.findFileByName(this.session,parent.id, name);
     ...
     */
    async getFileOfPath (session,path) {
      const paths = await this.getPaths(this.session,path);
	 ...
	 */
	async mkdir(session, path) {
	    debug(`mkdir(${path})`);

	    path = path.replace(/\/+$/, "");
	    const absPath = this.toAbsolutePath(new GdfsPath(path));
	    const parentFolder = await this.getFileOfPath(session,absPath.getPathPart());
	    ...
	    const files = await gapi.findFileByName(this.session,parentFolder.id, pathname);
	    debug(`mkdir: files: ${JSON.stringify(files)}`);
	    if(files.length > 0) {
	        debug(`mkdir: The directory exists ${path}`);
	        return null;
	    }
	    const result = await gapi.createFile(this.session, parentFolder.id, pathname, gapi.mimeTypeFolder);
     ...
     */
    async rename(session, path, newName) {
        debug(`rename(${path})`);
        const absPath = this.toAbsolutePath(new GdfsPath(path));
        const parentFolder = await this.getFileOfPath(session,absPath.getPathPart());
        ...
        if(file.mimeType === gapi.mimeTypeFolder) {
            debug(`rename: The file is a folder ${path}`);
            return null;
        }


        const response = await this.session.drive.files.update({ 
            fileId: file.id, 
            requestBody: {
                name: newName
            }
     ...
     */
    async readdir(session, path,options) {
        path += path.match(/\/$/) ? "" : "/";
        const absPath = this.toAbsolutePath(new GdfsPath(path));
        ...
        const readFiles = async params => {
            ...
            const result = await gapi.getFileList(this.session,params);
            debug(`readdir: result.nextPageToken: ${result.nextPageToken}`);
            for(const file of result.files) {
                files.push(gapi.toUnifiedFileInfo(file));
            }
	...
	async readFile(session, path) {
	    ...
	    const files = await gapi.findFileByName(this.session,parentFolder.id, filename);
	    ...
	    return await gapi.downloadFile(this.session,file.id);
	}
	 ...
	 */
	async rmdir(session, path) {
	    debug(`rmdir(${path})`);
	    path = path.replace(/\/+$/, "");
	    const absPath = this.toAbsolutePath(new GdfsPath(path));
	    const parentFolder = await this.getFileOfPath(this.session,absPath.getPathPart());
	    ...
	    const dires = await gapi.findFolderByName(this.session,parentFolder.id, pathname);
	    ...
	    ///    dir.id === await gapi.getActualRootFolderId())
        if (dir.id === await gapi.getActualRootFolderId(this.session))
	    {
	        debug(`rmdir: The path is a parent ${path}`);
	        return null;
	    }
	    if(dir.mimeType !== gapi.mimeTypeFolder) {
    ...
    async unlink(session, path) {
        ...
        const response = await this.session.drive.files.delete({ fileId: file.id });
        const result = response.result;
        ///if(parentFolder.id === this.getCurrentFolderId()) {
        ///    await this.fireCwdUpdate();
        ///}
     ...
     * @returns {Promise<object>} The API response.
     */
    async writeFile(session, path, data,mimeType) {
        ...
        if(files.length === 0) {
            const file = await gapi.createFile(this.session,parentFolder.id, filename, mimeType);
            const result = await gapi.updateFile(this.session,file.id, mimeType, data);
            ///if(parentFolder.id === this.getCurrentFolderId()) {
            ///    await this.fireCwdUpdate();
            ///}
            return result;
