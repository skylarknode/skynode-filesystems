...
const {UnifileError} = require('skynode-vfs/lib/error');
...
class VolumesConnector {
   ...
   */
   ...
   archive(paths,to,options) {
      ...
      for (var i=0;i<paths.length;i++) {
         console.log("archive:path"+i+"=" + paths[i]);
         var src = parsePath(paths[i]);
         if (!connectorName) {
            connectorName = src.connectorName;
         } else if (connectorName != src.connectorName) {
            return Promise.reject(new Error("Invalid Operation"));
         }
         paths[i] = src.path;
      }
      if (typeof to == "string") {
         console.log("archive:to=" + to);
         var dest = parsePath(to);
         if (connectorName != dest.connectorName) {
            return Promise.reject(new Error("Invalid Operation"));
         }
         to = dest.path;
      }
      return this.callMethod(connectorName,  'archive', paths,to,options);
   }
   ...
   * Create a file.
   * @param {Object} session - Object where session data will be stored
   * @param {string} path - Path of the file to create. Must be relative to the root of the service.
   * @return {external:Promise<null>} an empty promise
   */
   mkfile( path) {
      var parsed = parsePath(path);
      return this.callMethod(parsed.connectorName,  'mkfile', parsed.path);
   }
   ...
   */
   move(srcPath, destPath) {
      ...
      return this.callMethod(src.connectorName,  'move', src.path,dest.path).then(function(info){
            normalizeInfo(info,dest.connectorName)
            return info;
      });
   }
   ...
   */
   readdir( path) {
      if (path == "/") {
         var connectorNames = this.listConnectors(),
             dirs = [];
         connectorNames.forEach(function(volumeId){
            dirs.push({
               name : volumeId,
   ...
   */
   writeFile( path, content) {
      ...
      console.dir(parsed);
      return this.callMethod(parsed.connectorName,  'writeFile', parsed.path, content).then(function(info){
               normalizeInfo(info,parsed.connectorName)
               return info;
         });         
   }
   ...
   */
   remove( path) {
      var parsed = parsePath(path);
      return this.callMethod(parsed.connectorName,  'remove', parsed.path);
   }
   ...
   */
   rename( path, newName) {
      var parsed = parsePath(path);
      return this.callMethod(parsed.connectorName,  'rename', parsed.path, newName).then(function(info){
         normalizeInfo(info,parsed.connectorName)
         return info;
      });
   }
   ...
   */
   ...
   thumbnail(path,options) {
      var parsed = parsePath(path),
          originalEncode = options.encode;
      options.encode = function(path) {
         return originalEncode("/" + parsed.connectorName + path);
      }
      return this.callMethod(parsed.connectorName,  'thumbnail', parsed.path,options);
   }
