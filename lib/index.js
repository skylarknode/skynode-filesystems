...
class FileSystem {
   ...
   */
   static use(ctor,options) {
      if(!ctor) throw new Error('Connector cannot be undefined');
      if(!ctor.name) throw new Error('Connector must have a name');
      options = options || {}
      this[connectors].set(connector.name.toLowerCase(), {
         ctor,
         options
      });
   }
