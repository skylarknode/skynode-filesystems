...
const Unifile = require('../../lib/index-1');
...
unifile.use(gdriveconnector);

// Expose connector methods
app.post('/:connector/authorize', function(req, res) {
	if(req.body != null) {
		if(req.session.unifile.http)
			req.session.unifile.http.userAddress = req.body.userAddress;
		else
			req.session.unifile.http = req.body;
	}
